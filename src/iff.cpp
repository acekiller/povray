...
*
* $File: //depot/povray/3.5/source/iff.cpp $
* $Revision: #11 $
* $Change: 1817 $
...
static int read_byte(POV_ISTREAM *f);
static int read_word(POV_ISTREAM *f);
static long read_long(POV_ISTREAM *f);
static void Read_Chunk_Header(POV_ISTREAM *f, CHUNK_HEADER *dest);
    ...
    switch (IFF_SWITCH_CAST Chunk_Header.name)
    {
      ...
      case BODY:
        ...
        for (i = 0; i < Image->iheight; i++)
        {
          if (Image->Colour_Map == NULL)
          {
            Image->data.rgb8_lines[i].red = (unsigned char *)POV_MALLOC((size_t) Image->iwidth, "IFF image line");
            Image->data.rgb8_lines[i].green = (unsigned char *)POV_MALLOC((size_t) Image->iwidth, "IFF image line");
            Image->data.rgb8_lines[i].blue = (unsigned char *)POV_MALLOC((size_t) Image->iwidth, "IFF image line");
          }
          else
          {
