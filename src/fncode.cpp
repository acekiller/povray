...
*
* $File: //depot/povray/3.5/source/fncode.cpp $
* $Revision: #76 $
* $Change: 1817 $
...
FNCode::FNCode(FunctionCode *f, bool is_local, char *n)
{
	...
	function->filename = POV_STRDUP(Token.Filename);
	function->line = Token.Token_Line_No;
	function->flags = 0;
...
void FNCode_Copy(FunctionCode *f, FunctionCode *fnew)
{
	...
	if(f->program != NULL)
	{
		...
		memcpy(fnew->program, f->program, sizeof(Instruction) * f->program_size);
	}
	if(f->name != NULL)
	{
...
void FNCode::compile_member(char *name)
{
	...
	else if((strcmp(name, "gray") == 0) || (strcmp(name, "grey") == 0))
	{
		...
		compile_instruction(OPCODE_LOAD, 1, 5, return_parameter_sp);
		compile_instruction(OPCODE_MULI, 0, 5, POVFPU_AddConstant(0.297));
		compile_instruction(OPCODE_LOAD, 1, 0, return_parameter_sp + 1);
		compile_instruction(OPCODE_MULI, 0, 0, POVFPU_AddConstant(0.589));
		compile_instruction(OPCODE_ADD, 0, 5, 0);
		compile_instruction(OPCODE_LOAD, 1, 0, return_parameter_sp + 2);
		compile_instruction(OPCODE_MULI, 0, 0, POVFPU_AddConstant(0.114));
		compile_instruction(OPCODE_ADD, 0, 5, 0);
	}
...
void FNCode::compile_seq_op(ExprNode *expr, unsigned int op, DBL neutral)
{
	...
	var_sp = stack_pointer + max(((int)(level)) - 2, 0) + min(level + 1, 3);
...
unsigned int FNCode::compile_instruction(unsigned int op, unsigned int rs, unsigned int rd, unsigned int k)
{
	...
	{
		max_program_size = max(MAX_K, ((unsigned int)max_program_size) + 256);
