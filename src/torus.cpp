...
*
* $File: //depot/povray/3.5/source/torus.cpp $
* $Revision: #11 $
* $Change: 1817 $
  ...
  if ((max_i = intersect_torus(Ray, (TORUS *)Object, Depth)) > 0)
  {
    for (i = 0; i < max_i; i++)
    {
      if ((Depth[i] > DEPTH_TOLERANCE) && (Depth[i] < Max_Distance))
      {
        ...
        {
 		UV_VECT uv;
		CalcUV((TORUS *)Object, IPoint, uv);
         push_uv_entry(Depth[i], IPoint, uv, Object, Depth_Stack);
...
void Torus_UVCoord(UV_VECT Result, OBJECT *Object, INTERSECTION *Inter)
{
