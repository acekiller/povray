...
bool ot_dist_traverse(OT_NODE *subtree, VECTOR point, int bounce_depth, long (*function)(OT_BLOCK *block, void *handle1), void *handle)
...
*
******************************************************************************/
bool ot_save_tree(OT_NODE *root, POV_OSTREAM *fd)
{
  bool retval = false;
...
ot_write_block/* must be passed as void * for compatibility */(OT_BLOCK *bl, void *fd)
{
  ...
  {
    ((POV_OSTREAM *)fd)->printf("C%ld\t%g\t%g\t%g\t%02lx%02lx%02lx\t%.4f\t%.4f\t%.4f\t%g\t%g\t%02lx%02lx%02lx\n", /* tw */
...
{
  bool retval, got_eof;
  long line_num, tempdepth, tx, ty, tz, goodreads;
  int count, goodparse ;
  DBL brightness;
  OT_BLOCK bl;
  OT_BLOCK *new_block;
