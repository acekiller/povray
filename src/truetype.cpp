...
* Unicode character fixed, Feb. 2000 [trf]
*
* $File: //depot/povray/3.5/source/truetype.cpp $
* $Revision: #30 $
* $Change: 1817 $
...
static BYTE tag_TTCFontFile[]    = "ttcf"; /* */
...
static SHORT readSHORT (POV_ISTREAM *infile, int line, char *file);
static USHORT readUSHORT (POV_ISTREAM *infile, int line, char *file);
static LONG readLONG (POV_ISTREAM *infile, int line, char *file);
static ULONG readULONG (POV_ISTREAM *infile, int line, char *file);
...
void ProcessNewTTF(OBJECT *object, char *filename, UCS2 *text_string, DBL depth, VECTOR offset)
{
	...
	{
		Warning(0, "Text may not be displayed as expected.\n"
  ...
  if(ffile->fp!=NULL)
  {
    POV_DELETE(ffile->fp, POV_ISTREAM);
    ffile->fp = NULL;
  }
}
...
static FontFileInfo *OpenFontFile(char *filename)
{
  ...
  if(fontlist != NULL)
  {
    ...
    {
      #ifdef TTF_DEBUG
      Debug_Info("Using cached font info for %s", fontlist->filename);
      #endif
    }
  }
  else
...
void FreeFontInfo()
{
  ...
  {
    if (oldfont->fp != NULL)
      POV_DELETE(oldfont->fp, POV_ISTREAM);
...
static void ProcessKernTable(FontFileInfo *ffile, long kern_table_offset)
{
  ...
  for (i = 0; i < kern_table->nTables; i++)
  {
    ...
    else
    {
       ...
       */
      ffile->fp->seekg((long)(length - 6), POV_SEEK_CUR) ;
...
static USHORT ProcessFormat0Glyph(FontFileInfo *ffile, unsigned int search_char)
{
  ...
  ffile->fp->seekg ((long)search_char, POV_SEEK_CUR) ;
...
static USHORT ProcessFormat6Glyph(FontFileInfo *ffile, unsigned int search_char)
{
  ...
  {
    ffile->fp->seekg ((long)(search_char - firstCode), POV_SEEK_CUR) ;
    glyph_index = READUSHORT(ffile->fp);
  }
  else
    glyph_index = 0;
