...
*
* $File: //depot/povray/3.5/source/pov_mem.cpp $
* $Revision: #15 $
* $Change: 1817 $
...
static char *mem_prefill_string = MEM_PREFILL_STRING;
static size_t mem_prefill_string_len = 0;
static char *mem_clear_string = MEM_CLEAR_STRING;
static size_t mem_clear_string_len = 0;
#endif
...
void *pov_realloc(void *ptr, size_t size, char *file, int line, char *msg)
{
  ...
  #if defined(MEM_PREFILL)
    memptr = (char *)block + NODESIZE;
    for(i = size; i < oldsize - NODESIZE; i++)
  ...
  #if defined(MEM_PREFILL)
    memptr = (char *)block + NODESIZE;
    for(i = oldsize - NODESIZE; i < size; i++)
       memptr[i] = mem_prefill_string[i % mem_prefill_string_len];
  #endif
...
void mem_release()
{
  ...
  if (p != NULL && (p->poolno == poolno))
    f = POV_NEW_OSTREAM(MEM_LOG_FNAME, POV_File_Data_LOG, true);
#endif /* MEM_TRACE */
