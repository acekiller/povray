...
*
* $File: //depot/povray/3.5/source/warps.cpp $
* $Revision: #11 $
* $Change: 1817 $
...
void Warp_Classic_Turb (VECTOR TPoint, VECTOR EPoint, TPATTERN *TPat)
{
   ...
   if (Warp!=NULL)
   while (Warp->Next_Warp != NULL)
   {
     Warp = Warp->Next_Warp;
   }

   if(Warp->Warp_Type == CLASSIC_TURB_WARP)
   {
    if (!(
        (TPat->Type == MARBLE_PATTERN) ||
        (TPat->Type == NO_PATTERN)     ||
        (TPat->Type == WOOD_PATTERN)))
    {
      Turb=(TURB *)Warp;
...
WARP *Copy_Warps (WARP *Old)
{
  ...
  if (Old != NULL)
  {
    ...
    {
       case CYLINDRICAL_WARP:
         memcpy(New,Old,sizeof(CYLW));
         break;
     
       case PLANAR_WARP:
         memcpy(New,Old,sizeof(PLANARW));
         break;
     
       case SPHERICAL_WARP:
         memcpy(New,Old,sizeof(SPHEREW));
         break;
     
       case TOROIDAL_WARP:
         memcpy(New,Old,sizeof(TOROIDAL));
         break;
